...
@ignore_warnings
class mesh2tiff:
    def __init__(self, inputDir, outputDir, inputFile, chunkSize):
        ...
        # Run exportRaster using multiprocessinng pool, and imput_list
        logger.info('Run exportRaster in Pool, with inputs_list')
        #with Pool(processes=4) as pool:
        #    outputs_list = pool.map(self.exportRaster, [(input_q, input_list) for input_list in inputs_list], chunksize=chunkSize)
        #    pool.close()
        #    pool.terminate()
        with Pool(processes=4) as pool:
            try:
               outputs_list = pool.map(self.exportRaster, [(input_q, input_list) for input_list in inputs_list], chunksize=int(chunkSize))
    ...
    def exportRaster(self, inputsList):
        # Get inputs from inputsList
        inputDir = inputsList[1][0]
        inputFile = inputsList[1][1]
        outputDir = inputsList[1][2]
        outputFile = inputsList[1][3]
        inputExtent = inputsList[1][4]
        timeStep = inputsList[1][5]
        mapUnitPP = inputsList[1][6]
...
def main(args):
    ...
    if args.chunkSize is None:
        chunkSize = '2' 
    elif args.chunkSize is not None:
        chunkSize = args.chunkSize
    else:
        sys.exit('Incorrect chunkSize')
    ...
    if os.path.exists(inputDir+inputFile):
        ...
        mesh2tiff(inputDir, outputDir, inputFile, chunkSize)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--chunkSize", help="Chunksize for pool", action="store", dest="chunkSize", required=False)
